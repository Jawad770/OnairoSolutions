...
function styles() {
  return `
    ...
    *{box-sizing:border-box} body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:radial-gradient(circle at 20% -10%,#123 0%,#070b14 45%);color:var(--text)}
    a{color:inherit;text-decoration:none} .app{display:grid;grid-template-columns:260px 1fr;min-height:100vh}
    .side{position:sticky;top:0;height:100vh;padding:22px;border-right:1px solid var(--line);backdrop-filter:blur(14px);background:rgba(10,15,30,.7);overflow-y:auto;z-index:40}
    .brand{font-weight:800;letter-spacing:.02em;margin-bottom:6px}.muted{color:var(--muted);font-size:12px}
    .nav{display:grid;gap:8px}.nav a{padding:10px 12px;border:1px solid transparent;border-radius:12px;color:var(--muted);transition:background .18s ease,color .18s ease,border-color .18s ease}
    .nav a:hover,.nav a.active{border-color:var(--line);color:var(--text);background:rgba(148,163,184,.08)}
    .main{padding:20px;min-width:0}
    .top{position:sticky;top:0;z-index:30;border:1px solid var(--line);background:rgba(15,23,42,.75);backdrop-filter:blur(12px);padding:12px 16px;border-radius:14px;display:flex;justify-content:space-between;align-items:center;gap:12px}
    ...
    .card{border:1px solid var(--line);background:var(--card);backdrop-filter:blur(18px);padding:14px;border-radius:14px}
    .k{font-size:12px;color:var(--muted)} .v{font-size:24px;font-weight:800;margin-top:6px}
    .panel{margin-top:14px;border:1px solid var(--line);background:var(--card);border-radius:14px;padding:14px}
    .panel-head{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:10px}
    table{width:100%;border-collapse:collapse} th,td{padding:10px;border-bottom:1px solid var(--line);text-align:left;font-size:13px;vertical-align:middle}
    ...
    .kanban{display:grid;grid-template-columns:repeat(7,minmax(180px,1fr));gap:10px;overflow:auto}
    ...
    .form{max-width:420px;margin:6vh auto;border:1px solid var(--line);background:rgba(10,15,30,.72);padding:22px;border-radius:16px;backdrop-filter:blur(18px)}
    input,select,textarea{width:100%;padding:10px;border-radius:10px;border:1px solid var(--line);background:rgba(2,6,23,.5);color:var(--text);font:inherit}
    ...
    .error{color:#fca5a5;font-size:13px;margin-bottom:8px;border:1px solid rgba(248,113,113,.35);background:rgba(248,113,113,.08);padding:10px;border-radius:10px}
    .notice{color:#6ee7b7;font-size:13px;margin-bottom:8px;border:1px solid rgba(16,185,129,.35);background:rgba(16,185,129,.08);padding:10px;border-radius:10px}
    ...
    @media (max-width:1100px){
      ...
      .side{position:fixed;left:0;top:0;width:min(82vw,300px);transform:translateX(-102%);transition:transform .26s ease;box-shadow:0 24px 60px rgba(0,0,0,.5)}
...
<body>
  ...
  <script>
    (function(){
      var btn=document.getElementById('drawerBtn'),scrim=document.getElementById('scrim');
